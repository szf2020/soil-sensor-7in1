...
REST API для интеграции с JXCT Soil Sensor v3.12.0
...
- [Связанная документация](#Svyazannaya-dokumentatsiya)
- [Содержание](#Soderzhanie)
- [🆕 API v3.12.0 Endpoints](#api-v3.12.0-endpoints)
- [Доступ к API](#Dostup-k-api)
  - [Таблица актуальных эндпоинтов (API v3.12.0)](#Tablitsa-aktualnyh-endpointov-api-v3.12.0)
...
### 🎯 **Основные API v3.12.0 маршруты**

Новая архитектура API v3.12.0 с улучшенной структурой и функциональностью:

| Endpoint | Метод | Описание | Статус |
|----------|-------|----------|--------|
| `/api/v3.12.0/sensor` | GET | Данные датчика с timestamp | ✅ Активен |
| `/api/v3.12.0/system/health` | GET | Состояние системы | ✅ Активен |
| `/api/v3.12.0/system/status` | GET | Статус сервисов | ✅ Активен |
| `/api/v3.12.0/system/reset` | POST | Сброс настроек | ✅ Активен |
| `/api/v3.12.0/system/reboot` | POST | Перезагрузка | ✅ Активен |
| `/api/v3.12.0/config/export` | GET | Экспорт конфигурации | ✅ Активен |

### 📊 **Структура ответа API v3.12.0**

**GET /api/v3.12.0/sensor:**
```json
{
...
| Метод | Путь | Описание |
|-------|------|----------|
| GET   | `/api/v3.12.0/sensor`         | Основные данные датчика (JSON) |
| GET   | `/api/v3.12.0/system/health`  | Полная диагностика устройства  |
| GET   | `/api/v3.12.0/system/status`  | Краткий статус сервисов        |
| POST  | `/api/v3.12.0/system/reset`   | Сброс настроек (307 на `/reset`) |
| POST  | `/api/v3.12.0/system/reboot`  | Перезагрузка (307 на `/reboot`) |
| GET   | `/api/v3.12.0/config/export`  | Скачать конфигурацию (JSON, без паролей) |
| POST  | `/api/v3.12.0/config/import`  | Импорт конфигурации            |
...
| GET   | `/api/sensor`            | DEPRECATED alias → `/api/v3.12.0/sensor` |
| GET   | `/api/config/export`     | DEPRECATED alias → `/api/v3.12.0/config/export` |
| POST  | `/api/config/import`     | DEPRECATED alias → `/api/v3.12.0/config/import` |
