...
REST API для интеграции с JXCT Soil Sensor v3.13.1
...
- Устройство поддерживает как новые маршруты `/api/v3.13.1/*`, так и совместимые с более старыми тестами/клиентами пути:
  ...
  - Новые эквиваленты: `GET /api/v3.13.1/system/health`, `GET /api/v3.13.1/sensor`.

Рекомендация: для новых интеграций использовать `/api/v3.13.1/*`. Для обратной совместимости перечисленные legacy пути остаются доступными.

## 📋 Содержание {#Soderzhanie}

- [Связанная документация](#Svyazannaya-dokumentatsiya)
- [Содержание](#Soderzhanie)
- [🆕 API v3.13.1 Endpoints](#api-v3.13.1-endpoints)
- [Доступ к API](#Dostup-k-api)
  - [Таблица актуальных эндпоинтов (API v3.13.1)](#Tablitsa-aktualnyh-endpointov-api-v3.13.1)
...
### 🎯 **Основные API v3.13.1 маршруты**

Новая архитектура API v3.13.1 с улучшенной структурой и функциональностью:

| Endpoint | Метод | Описание | Статус |
|----------|-------|----------|--------|
| `/api/v3.13.1/sensor` | GET | Данные датчика с timestamp | ✅ Активен |
| `/api/v3.13.1/system/health` | GET | Состояние системы | ✅ Активен |
| `/api/v3.13.1/system/status` | GET | Статус сервисов | ✅ Активен |
| `/api/v3.13.1/system/reset` | POST | Сброс настроек | ✅ Активен |
| `/api/v3.13.1/system/reboot` | POST | Перезагрузка | ✅ Активен |
| `/api/v3.13.1/config/export` | GET | Экспорт конфигурации | ✅ Активен |

### 📊 **Структура ответа API v3.13.1**

**GET /api/v3.13.1/sensor:**
```json
{
...
| Метод | Путь | Описание |
|-------|------|----------|
| GET   | `/api/v3.13.1/sensor`         | Основные данные датчика (JSON) |
| GET   | `/api/v3.13.1/system/health`  | Полная диагностика устройства  |
| GET   | `/api/v3.13.1/system/status`  | Краткий статус сервисов        |
| POST  | `/api/v3.13.1/system/reset`   | Сброс настроек (307 на `/reset`) |
| POST  | `/api/v3.13.1/system/reboot`  | Перезагрузка (307 на `/reboot`) |
| GET   | `/api/v3.13.1/config/export`  | Скачать конфигурацию (JSON, без паролей) |
| POST  | `/api/v3.13.1/config/import`  | Импорт конфигурации            |
...
| GET   | `/api/sensor`            | DEPRECATED alias → `/api/v3.13.1/sensor` |
| GET   | `/api/config/export`     | DEPRECATED alias → `/api/v3.13.1/config/export` |
| POST  | `/api/config/import`     | DEPRECATED alias → `/api/v3.13.1/config/import` |
