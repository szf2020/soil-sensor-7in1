                                                                   ...
                                                                   float pH)
{
    ...
    if (cropName == "tomato" || cropName == "томат") {
        ...
        // Томаты чувствительны к дефициту магния при высоком калии
        if (npk.potassium > 200.0F) {  // Снизили порог с 350 до 200
        ...
        if (npk.nitrogen > 150.0F && npk.phosphorus < 100.0F) {
            recommendations += "🍅 Томаты нуждаются в сбалансированном питании. ";
            recommendations += "Рекомендуется: увеличить фосфор для развития корневой системы\n";
        }
        
        // Томаты нуждаются в боре для качества плодов
        if (pH > 7.0F || npk.potassium > 300.0F) {
            recommendations += "🍅 Томаты требуют бор для предотвращения деформации плодов. ";
        ...
        // Огурцы нуждаются в магнии при высоком калии
        if (npk.potassium > 200.0F && npk.nitrogen < 150.0F) {
        ...
        if (npk.phosphorus > 100.0F) {  // Снижен с 250 до 100 для научной точности
            recommendations += "🌶️ Высокий фосфор может блокировать цинк у перца. ";
        ...
        if (pH < 6.5F) {  // Расширен с 6.0 до 6.5
            recommendations += "🌶️ Перец требует кальций для качества плодов. ";
            recommendations += "Рекомендуется: внести кальциевую селитру\n";
        }
        ...
        if (npk.nitrogen > 250.0F) {
            recommendations += "🥬 Высокий азот требует серу для синтеза белка у салата. ";
            recommendations += "Рекомендуется: внести сульфат аммония ((NH4)2SO4)\n";
        }
        
        // Салат чувствителен к дефициту железа
        if (pH > 7.0F) {
            recommendations += "🥬 Щелочная почва снижает доступность железа для салата. ";
        ...
        if (pH < 6.0F) {
            recommendations += "🍓 Клубника требует кальций для твердости ягод. ";
            recommendations += "Рекомендуется: внести кальциевую селитру (Ca(NO3)2)\n";
        }
        
        // Клубника нуждается в боре для опыления
        if (pH > 6.5F || npk.potassium > 200.0F) {
            recommendations += "🍓 Клубника требует бор для завязывания ягод. ";
            recommendations += "Рекомендуется: внести борную кислоту (H3BO3)\n";
        }
        
        // Клубника чувствительна к дефициту цинка
        if (npk.phosphorus > 80.0F) {
            recommendations += "🍓 Высокий фосфор может блокировать цинк у клубники. ";
            recommendations += "Рекомендуется: внести хелатный цинк (Zn-EDTA)\n";
        }
    }
        ...
        if (pH < 6.5F || npk.potassium > 250.0F) {
            recommendations += "🍎 Яблоня требует кальций для предотвращения горькой ямчатости. ";
            recommendations += "Рекомендуется: внести кальциевую селитру\n";
        }
        
        // Яблоня нуждается в боре для развития плодов
        if (pH > 7.0F) {
            recommendations += "🍎 Яблоня требует бор для нормального развития плодов. ";
            recommendations += "Рекомендуется: внести борную кислоту (H3BO3)\n";
        }
        
        // Яблоня чувствительна к дефициту цинка (розеточность)
        if (pH > 7.0F || npk.phosphorus > 60.0F) {
            recommendations += "🍎 Яблоня требует цинк против розеточности побегов. ";
            recommendations += "Рекомендуется: внести сульфат цинка (ZnSO4)\n";
        }
    }
        ...
        if (npk.potassium < 200.0F) {
            recommendations += "🍇 Виноград требует калий для накопления сахаров. ";
            recommendations += "Рекомендуется: внести сульфат калия (K2SO4)\n";
        }
        
        // Виноград нуждается в магнии против хлороза
        if (npk.potassium > 250.0F || pH > 7.5F) {
        ...
        if (pH > 7.0F) {
            recommendations += "🍇 Виноград требует бор для завязывания ягод. ";
            recommendations += "Рекомендуется: внести борную кислоту (H3BO3)\n";
        }
    }
        ...
        if (npk.nitrogen < 200.0F) {
            recommendations += "🌾 Пшеница требует азот для синтеза белка в зерне. ";
            recommendations += "Рекомендуется: внести азотные удобрения (NH4NO3)\\n";
        }
        
        // Пшеница нуждается в фосфоре для развития корневой системы
        if (npk.phosphorus < 50.0F) {
            recommendations += "🌾 Пшеница требует фосфор для развития корней. ";
        ...
        if (npk.nitrogen > 250.0F && pH > 7.0F) {
            recommendations += "🌾 Пшеница требует серу для синтеза белка. ";
            recommendations += "Рекомендуется: внести сульфат аммония ((NH4)2SO4)\\n";
        }
        
        // Пшеница нуждается в калии для устойчивости к болезням
        if (npk.potassium < 150.0F) {
            recommendations += "🌾 Пшеница требует калий для иммунитета. ";
    ...
    if (soilType == SoilType::PEAT) {
        recommendations += "🟫 Торфяные почвы могут содержать много азота, но мало фосфора. ";
        recommendations += "Рекомендуется: внести фосфорные удобрения\n";
    }
