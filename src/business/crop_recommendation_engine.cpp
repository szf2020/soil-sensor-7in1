                                                                   ...
                                                                   float pH)
{
    ...
    // 🌱 ДОБАВЛЯЕМ НЕДОСТАЮЩИЕ КУЛЬТУРЫ
        ...
        // Малина требует железо для предотвращения хлороза
        if (pH > 7.0F) {
            recommendations += "🍇 Малина требует железо для предотвращения хлороза. ";
            recommendations += "Рекомендуется: внести хелатное железо (Fe-EDTA)\n";
        }
        
        // Малина нуждается в боре для завязывания ягод
        if (pH > 6.8F || npk.potassium > 300.0F) {
            recommendations += "🍇 Малина требует бор для формирования ягод. ";
            recommendations += "Рекомендуется: внести борную кислоту (H3BO3)\n";
        }
        
        // Малина нуждается в цинке для роста побегов
        if (npk.phosphorus > 70.0F) {
            recommendations += "🍇 Малина требует цинк для роста побегов. ";
            recommendations += "Рекомендуется: внести хелатный цинк (Zn-EDTA)\n";
        }
    }
        ...
        // Кале требует много азота для роста листьев
        if (npk.nitrogen < 150.0F) {
            recommendations += "🥬 Кале требует много азота для интенсивного роста листьев. ";
            recommendations += "Рекомендуется: внести азотные удобрения (NH4NO3)\n";
        }
        
        // Кале нуждается в кальции для качества листьев
        if (pH < 6.5F) {
            recommendations += "🥬 Кале требует кальций для предотвращения некроза листьев. ";
            recommendations += "Рекомендуется: внести кальциевую селитру (Ca(NO3)2)\n";
        }
        
        // Кале чувствительна к дефициту магния
        if (npk.potassium > 300.0F || pH > 6.5F) {
            recommendations += "🥬 Кале требует магний для фотосинтеза. ";
            recommendations += "Рекомендуется: внести сульфат магния (MgSO4)\n";
        }
    }
