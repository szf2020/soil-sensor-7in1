                                                                   ...
                                                                   float pH)
{
    ...
    if (nk_antagonism < 0.8F) {
        recommendations += "⚠️ Высокий азот блокирует калий. Рекомендуется: ";
        recommendations += "уменьшить азотные удобрения, увеличить калийные\n";
    }
    
    // 2. pH-зависимые антагонизмы (pH измеряется!)
    if (pH > 7.5F) {
        recommendations += "⚠️ Щелочная почва усиливает P-Ca антагонизм. Рекомендуется: ";
        recommendations += "подкислить почву, внести хелатные формы фосфора\n";
    }
    
    // 3. P vs K взаимодействие (измеряются оба!)
    float pk_ratio = npk.phosphorus / npk.potassium;
    if (pk_ratio > 0.8F) {
        recommendations += "⚠️ Высокий фосфор может блокировать калий. Рекомендуется: ";
        recommendations += "уменьшить фосфорные удобрения, увеличить калийные\n";
    }
    ...
    if (npk.potassium > 400.0F) {
        recommendations += "💡 Высокий калий может блокировать магний. Рекомендуется: ";
        recommendations += "внести сульфат магния (MgSO4), уменьшить калийные удобрения\n";
    }
    
    // 5. N + S синергизм (N измеряется, S - нет)
    if (npk.nitrogen > 300.0F) {
        recommendations += "💡 Высокий азот требует серу для синтеза белка. Рекомендуется: ";
        recommendations += "внести сульфат аммония ((NH4)2SO4) или элементарную серу\n";
    }
    
    // 6. P vs Zn антагонизм (P измеряется, Zn - нет)
    if (npk.phosphorus > 200.0F) {
        recommendations += "💡 Высокий фосфор может блокировать цинк. Рекомендуется: ";
        recommendations += "внести хелатный цинк (Zn-EDTA), уменьшить фосфорные удобрения\n";
    }
    
    // 7. Ca + B синергизм (ни один не измеряется, но pH влияет)
    if (pH < 6.0F) {
        recommendations += "💡 Кислая почва может снижать доступность кальция и бора. Рекомендуется: ";
        recommendations += "известковать почву, внести бор в хелатной форме\n";
    }
